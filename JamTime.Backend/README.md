...
To update the database use this command from the Web project folder (replace `JamTime.Backend` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../JamTime.Backend.Infrastructure/JamTime.Backend.Infrastructure.csproj -s JamTime.Backend.Web.csproj
```
